...
8. Push your generic image (the one with the root user) to some registry if not done already.
   This will be handy for you, for sharing it with your teammates, and when you open-source your project later.
    ...
    3. Add this link to the TODO ADD PULL_IMAGE_NAME in
       ...
       e.g., ic-registry.epfl.ch/your-lab/your-gaspar/claire-intro_.)
...
```bash
git clone <HTTPS/SSH> claire-intro
cd claire-intro
```
...
./template.sh run python -m claire_intro.template_experiment some_arg=some_value
...
```bash
mv claire-intro claire-intro-tmp
mkdir claire-intro
mv claire-intro-tmp claire-intro/dev
...
E.g., you can mount it at `/project/claire-intro` and specify `PROJECT_ROOT_AT=/project/claire-intro`.
...
export PROJECT_ROOT_AT=/project/claire-intro
apptainer run \
    -c \
    ...
    --env WANDB_API_KEY="" \
    --nv claire-intro_amd64-cuda-root-latest.sif
